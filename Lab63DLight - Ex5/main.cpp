...
int main(void)
{
	...
	window = glfwCreateWindow(WIDTH, HEIGHT, "Simon's 3D OpenGL Window", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return -1;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//cameraFront = glm::vec3(0.0f, dx-0.25f, -1.0f);

		//Ex5 different up vectors
		GLfloat dx = glm::abs(glm::sin(time) * 3.0f);
		//cameraUp = glm::vec3(1.0f, 2.0f, 0.0f);
		cameraUp = glm::vec3(time, 2.0f, dx);
		...
		//Ex5 Alt translate back and forth using FOV (Field of View)
		//GLfloat dx = glm::abs(glm::sin(time) * 0.5f);
		//projection = glm::perspective(dx + 45, (GLfloat)WIDTH / (GLfloat)HEIGHT, 0.1f, 100.0f);
